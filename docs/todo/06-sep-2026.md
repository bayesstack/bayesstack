...
## 17. Production-Hardened Relational Invariants & Architectural Enhancements

Following architectural review, the data model has been elevated with the following production invariants:

### 1. Database-Level Compound Foreign Key Tenant Isolation (P0)
- Every institutional table carries `UNIQUE (tenant_id, id)`.
- All child relationships enforce compound foreign keys:
  `FOREIGN KEY (tenant_id, child_entity_id) REFERENCES parent_table (tenant_id, id) ON DELETE CASCADE`.
- This mathematically prevents cross-tenant data corruption at the database kernel level, eliminating reliance on application-layer checks alone.

### 2. Decoupled 3-Tier Identity Model
- `users`: Global authenticated human identity (`id`, `email`, `hashed_password`, `is_superadmin`).
- `tenant_memberships`: University affiliation (`tenant_id`, `user_id`, `joined_at`, `is_active`).
- `tenant_roles`: Role authorization per membership (`'admin'`, `'faculty'`, `'learner'`, `'dept_chair'`).
- A single human can now seamlessly be Faculty at Ashoka and a Learner at Bayes Institute without credential duplication or role conflicts.

### 3. Consistent Publication Tuple Enforcement
- `course_publications` enforces `UNIQUE (tenant_id, university_course_id, id)` and a unique partial index for active status:
...
- `course_offerings` enforces consistent tuple binding:
...
- This ensures an institutional course offering can only bind to an approved publication snapshot belonging to that exact course and tenant.

### 4. Explicit Detached Snapshot Semantics for Copy-on-Write (CoW) Forks
- Once a tenant forks a chapter, the resulting record in `university_chapters` is a permanently detached, independent snapshot.
- Lineage columns (`source_library_chapter_id`, `source_library_version`, `forked_at`) exist strictly for provenance and visual diff explainability in faculty authoring tools.
- Upstream platform updates never trigger silent auto-mutations on institutional forks.

### 5. Content Namespace in Learner Mastery & Assessment Submissions
- `learner_concept_progress` adds `content_type VARCHAR(16) NOT NULL` (`'library'` | `'university'`) alongside `(concept_id, concept_version)`.
- `assessment_submissions` adds `studio_type VARCHAR(16) NOT NULL` (`'library'` | `'university'`) alongside `(studio_instance_id, studio_version)`.

### 6. Extended Academic Standing & Registration Tracking
- `student_academic_profiles`: Tracks institutional student number (roll number), academic standing (`'good_standing'`, `'academic_probation'`, `'dean_list'`), cumulative GPA, and credits earned.
- `section_enrollments`: Adds `registration_type` (`'standard'`, `'repeat_for_grade'`, `'audit'`) and `attempt_number` to model full academic registrar lifecycles.

---

## 18. Learner Experience Persistence Extension (Implemented 21 September 2026)

The variable-backed learner UI exposed five operational data gaps. Migration `0014_learner_experience_state` closes them without moving catalog content into mutable learner tables:

| Learner UI data | Canonical storage | Design rule |
| :--- | :--- | :--- |
| Career or skill goal | `learner_goals` | Tenant-scoped learner state; only one primary goal per learner. |
| Personal learning course | `personal_course_enrollments` -> immutable `catalog_courses(id, version)` | Separate from official registrar-managed `enrollments`. |
| Activity completion, resume position, draft checkpoint, bookmark | `learner_activity_progress` | Exactly one academic or personal context; the compound FK proves learner ownership. |
| Dated class, lab, deadline, exam, or office-hours item | `course_schedule_events` | May apply to an entire offering or one section; the section must belong to the same offering. |
| Offering-specific syllabus links, recordings, and readings | `course_offering_resources` | Supplements a frozen publication instead of mutating it. |

Existing tables remain authoritative for the rest of the screen:

- Course code, title, description, chapters, concepts, activity order, and studio descriptor: catalog/institution content plus `course_publications`.
- Term, credits, section, and instructor: `academic_terms`, `course_offerings`, `course_sections`, and `section_staff` joined to `users`.
- Concept mastery, assessed attempts, and grades: `learning_progress`, `assessment_submissions`, and `course_grades`.
- Recurring weekly meeting pattern: `course_sections.schedule_info`; dated occurrences belong in `course_schedule_events`.

`On track`, `Ahead`, and `Needs attention` remain derived read-model fields. They are calculated from required activity progress, scheduled obligations, term dates, and last access rather than stored as mutable labels.
