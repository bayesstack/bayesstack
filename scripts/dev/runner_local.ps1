...
function Warmup-LocalFrontends([string[]]$services) {
  $frontendPorts = @{
    landing = 3000
    learner = 3001
    faculty = 3002
  ...
  foreach ($service in $services) {
    ...
    for ($attempt = 1; $attempt -le 60; $attempt++) {
      try {
        ...
        # The dev server can be listening before its first route is compiled.
      }
      Start-Sleep -Seconds 2
    }

    if ($ready) {
      Write-LogSuccess "$service is compiled and ready on port $port."
    } else {
      Write-LogWarn "$service did not finish warming up within 120 seconds; it may still compile on first use."
    }
  }
}
...
function Run-LocalMode([string]$root, [string[]]$services) {
  ...
  Warmup-LocalFrontends $services
